...
public class AdminLogin extends JFrame {
    ...
    AdminLogin(){
    	if(s==null) {
	        ...
	        LoginLable=new JLabel("Admin Login");
    ...
    private void login(String userName,String password) {
        ...
        if(Pattern.matches("^[0-9]{2}[\\w]{7}$",userName))
        {
                 
	                try {
	                    Connection conn = Database.getConnection();
	
	                    PreparedStatement st = conn.prepareStatement("Select department from HostelWardens where RegisterNo=? and Password=?");
	                    ...
	                    if (rs.next()) {
	                        ...
	                        s = new AdminPanel(userName,department);
	                    } else {
	                        l1.setForeground(Color.red);
